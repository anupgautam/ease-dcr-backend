...
from DCRUser.views import (
    ...
    get_admin_user_id,
    # make_user_present
    get_all_lower_level_users_area,
...
router.register(r"company-user", CompanyUserViewset, basename="company_user")
router.register(
    r"company-user-without-pagination",
    CompanyUserWithoutPagination,
    basename="company_user_without_pagination",
)
router.register(
    r"company-user-role", CompanyUserRoleViewset, basename="company_user_role"
)
router.register(
    r"company-user-role-without-pagination",
    CompanyUserRoleViewsetWithoutPagination,
    basename="company_user_role_without_pagination",
)
router.register(
    r"company-user-attendance",
    CompanyUserAttendanceViewset,
    basename="company_user_attendance",
)
urlpatterns = [
    path(
        "get-upper-level-user-from-company-role/",
        get_upper_company_user_from_company_name_and_role_name,
    ),
    path(
        "get-all-upper-level-user-from-company-user-role/",
        get_all_the_upper_level_users_from_company_user_role_id,
    ),
    path("get-executives-from-user-id/", get_executives_from_user_id),
    path(
        "get-all-lower-level-users-from-company-user-role-id/",
        get_all_the_lower_level_users_from_company_user_role_id,
    ),
    path("get-lower-level-areas/", get_all_lower_level_users_area),
