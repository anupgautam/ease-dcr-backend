...
class MpoWiseLeaveApplicationSerializers(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # leave_application_instance.submit_to = application_data['submit_to']
    ...
    def validate_application_id(self, attrs):
        ...
        if not attrs['leave_from']:
            raise serializers.ValidationError("leave from date is mandatory")
        if not attrs['leave_to']:
            raise serializers.ValidationError("Leave to date is mandatory.")
        if (
            attrs['leave_from'] < date(nepali_today.year, nepali_today.month, nepali_today.day) 
            or attrs['leave_to'] < date(nepali_today.year, nepali_today.month, nepali_today.day)):
            raise serializers.ValidationError('Invalid leave date')
        return attrs
